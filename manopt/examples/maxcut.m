...
% is the
% symmetric adjacency matrix of the grap of the graph (A(i, j) = A(j, i) is the weight
...
% Laplacian of the graph L and an initial guess Y. L is nxn and Y is nxr.
    ...
    options.verbosity = 2;
%     [Y, Ycost, info] = trustregions(problem, Y, options);
     [Y, Ycost, info] = pso(problem, Y, options); %#ok<ASGLU>
